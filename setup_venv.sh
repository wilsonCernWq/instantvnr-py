#!/usr/bin/env bash
# Set up the Python environment for instantvnr.
#
# Usage:
...
#   4. Runs `uv pip install .[test]` which builds tinycudann (from git source)
#      and the C++ extension (cmake via scikit-build-core) in one step
...
TAG=$(cuda_tag)
echo "[info] CUDA tag: $TAG  |  CUDA_HOME: $CUDA_HOME"
...
# ── Install instantvnr + all deps (including tinycudann from source) ──────────
# uv resolves tinycudann via [tool.uv.sources] (git) and injects torch +
# setuptools into its build sandbox via [tool.uv.extra-build-dependencies].
# TCNN_CUDA_ARCHITECTURES is read by tinycudann's setup.py to select GPU arch.
echo "[info] Building instantvnr and dependencies (tinycudann will be built from source)..."
TCNN_CUDA_ARCHITECTURES="$SM" CMAKE_CUDA_ARCHITECTURES="$SM" UV_INDEX="pytorch=$TORCH_INDEX" \
    uv pip install -v .[test] --config-settings "cmake.define.CMAKE_CUDA_ARCHITECTURES=$SM"
