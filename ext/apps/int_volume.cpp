...
public:
  ...
  TransactionalValue<float> param_time;
  TransactionalValue<float> param_theta;
  TransactionalValue<float> param_phi;
  ...
  std::atomic<bool> print_camera_params = false;
...
public:
    ...
    , args(commandline)
  {
    ...
    if (args.has_simple_volume()) {
      ...
      // {
      //   std::cout << "[vnr] # of inference blobs = " << vnrNeuralVolumeGetNumberOfBlobs(volume) << std::endl;
      //   for (int i = 0; i < vnrNeuralVolumeGetNumberOfBlobs(volume); ++i) { 
      //     vnrNeuralVolumeDecodeProgressive(volume);
      //   }
      // }
      mode = NEURAL_VOLUME;
    }
  ...
  void initialize_transfer_function_widget()
  {
    ...
    // ASSERT_THROW(vnrVolumeGetValueRange(volume) == range, "expecting the same value range");
  ...
  void background_work()
  {
    ...
    if (mode == NEURAL_VOLUME) {
      if (save_params) {
        vnrNeuralVolumeSerializeParams(volume, "params.json");
        save_params = false;
      }
      if (load_params) {
        vnrNeuralVolumeSetParams(volume, "params.json");
        load_params = false;
      }
    }
    ...
    if (param_time.update()) {
      vnrRendererSetParams(renderer, param_time.get());
      frame_reset = true;
    }
    if (param_theta.update() || param_phi.update()) {
      vnrRendererSetParams(renderer, param_theta.get(), param_phi.get());
      frame_reset = true;
    }
    ...
    if (print_camera_params) {
      std::cout << "camera position: " << cameraFrame.get_position()  << std::endl;
      std::cout << "origin: " << cameraFrame.get_poi()  << std::endl;
      std::cout << "Up vector: " << cameraFrame.get_accurate_up()  << std::endl;
      print_camera_params = false;
    }
  ...
  void gui()
  {
    ...
    if (ImGui::Begin("Control Panel", NULL)) {
      ...
      static float gui_time = 0.5;
      if (ImGui::SliderFloat("HINR Time", &gui_time, 0.0f, 1.f, "%.3f")) {
        param_time = gui_time;
      }
      static float gui_theta = 0.0;
      if (ImGui::SliderFloat("HINR Theta", &gui_theta, 0.0f, 1.f, "%.3f")) {
        param_theta = gui_theta;
      }
      static float gui_phi = 0.0;
      if (ImGui::SliderFloat("HINR Phi", &gui_phi, 0.0f, 1.f, "%.3f")) {
        param_phi = gui_phi;
      }
      ...
      // record rendering & training results as JPG file
      if (ImGui::Button("Save Screen (JPG)")) {
        saveScreen("screenshot-" + std::to_string(image_count++) + ".jpg", "jpg");
      }
      ImGui::SameLine();
      // record rendering & training results as PNG file
      if (ImGui::Button("Save Screen (PNG)")) {
        saveScreen("screenshot-" + std::to_string(image_count++) + ".png", "png");
      }

      if (ImGui::Button("Print Camera Params")) {
        print_camera_params = true;
      }
  ...
  void saveScreen(const std::string &fname, const std::string &extension, vec2i size, vec4f* pixels)
  {
    ...
    if (!success) {
      std::cerr << "Failed to write: " << fname << " (possibly missing the directory for the file)" << std::endl;
    }
  }

  void saveScreen(const std::string &filename, const std::string &extension, bool sync = true)
  {
    ...
    // if (!pause)
      view.access([&](const View& view) { 
        ...
        saveScreen("screenshots/" + fname, extension, view.size, view.pixels);
      });
...
main(int ac, char** av)
{
  ...
  std::cout << "[vnr] total time = " << total << std::endl;
